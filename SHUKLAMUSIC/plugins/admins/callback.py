...
# ──────────────────────────────────────────────────────────────────────────────
# ✅ CLOSE BUTTON HANDLERS (were missing — this is why close buttons didn't work)
# ──────────────────────────────────────────────────────────────────────────────

@app.on_callback_query(filters.regex(r"^close$") & ~BANNED_USERS)
async def close_message(_, callback: CallbackQuery):
    """Deletes the message when the ✖️ Close button is tapped."""
    try:
        await callback.answer()
        await callback.message.delete()
    except Exception:
        try:
            await callback.answer("Message already deleted.", show_alert=False)
        except Exception:
            pass


@app.on_callback_query(filters.regex(r"^forceclose ") & ~BANNED_USERS)
async def forceclose_message(_, callback: CallbackQuery):
    """Deletes track-selection cards (track_markup / slider / playlist / livestream)."""
        ...
        if len(parts) >= 2:
            ...
            if str(callback.from_user.id) != str(user_id):
                return await callback.answer(
                    "❌ Only the person who searched can close this.", show_alert=True
                )
    try:
        await callback.answer()
        await callback.message.delete()
        ...
        try:
            await callback.answer("Already deleted.", show_alert=False)
        except Exception:
            pass
